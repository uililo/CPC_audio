...
def getCheckpointData(pathDir):
    ...
    return os.path.abspath(data), defaultArgs
...
def loadModel(pathCheckpoints, gru_level=None, loadStateDict=True):
    models = []
    hiddenGar, hiddenEncoder = 0, 0
    for path in pathCheckpoints:
        print(f"Loading checkpoint {path}")
        _, locArgs = getCheckpointData(os.path.dirname(path))
        ...
        if doLoad:
            m_, hg, he = loadModel(locArgs.load, gru_level, loadStateDict=False)
            hiddenGar += hg
            hiddenEncoder += he
        else:
